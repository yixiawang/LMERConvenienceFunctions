bfFixefLMER_t.fnc<-function (model, item = FALSE, method = c("t", "llrt", "AIC", 
                                          "BIC", "relLik.AIC", "relLik.BIC"), threshold = NULL, t.threshold = NULL, 
          alphaitem = NULL, prune.ranefs = TRUE, set.REML.FALSE = TRUE, 
          keep.single.factors = FALSE, reset.REML.TRUE = TRUE, log.file = NULL) 
{
  if (length(item) == 0) {
  ...
  if (!method[1] %in% c("t", "llrt", "AIC", "BIC", "relLik.AIC", 
                        "relLik.BIC")) {
  ...
  if ("factor" %in% attributes(attributes(model@frame)$terms)$dataClasses & 
        max(c(0, apply(as.data.frame(model@frame[, names(attributes(attributes(model@frame)$terms)$dataClasses[attributes(attributes(model@frame)$terms)$dataClasses == 
                                                                                                                 "factor"])]), MARGIN = 2, FUN = function(x) length(levels(factor(x)))))) > 
        2) {
  ...
  for (coef in coefs) {
    ...
    smry.temp2 <- smry.temp2[smry.temp2[, 3] == max(smry.temp2[, 
                                                               3]), ]
  ...
  for (order in orders) {
    ...
    while (smry.temp2[smry.temp2[, 3] == min(smry.temp2[, 
      ...
      for (i in 1:length(hoi)) {
        tt[i] <- length(grep("FALSE", unique(fac %in% 
                                               unlist(strsplit(hoi[i], ":"))))) < 1
      }
      if (length(grep("TRUE", tt)) != 0) {
        ...
        if (method[1] %in% c("AIC", "BIC")) {
          m.temp.ic <- attributes(summary(m.temp))$AICtab[1, 
                                                          method[1]]
          model.ic <- attributes(summary(model))$AICtab[1, 
                                                        method[1]]
        ...
        if (method[1] %in% c("relLik.AIC", "relLik.BIC")) {
          ...
          reduction <- FALSE
          cat(paste("     ", ic, " simple = ", round(m.temp.ic), 
                    "; ", ic, " complex = ", round(model.ic), 
                    "; relLik = ", round(my.relLik), "\n", sep = ""))
          if (m.temp.ic <= model.ic) {
            reduction <- TRUE
          }
          if (!reduction) {
            if (my.relLik >= threshold) {
              cat(paste("     ", "relLik of ", round(my.relLik), 
                        " >= ", threshold, "\n", sep = ""))
              cat("     skipping term\n")
              keepers <- row.names(smry.temp2)
              cat("length =", length(keepers), "\n")
        ...
        if (reduction) {
          ...
          for (coef in coefs) {
            ...
            smry.temp3 <- smry.temp3[smry.temp3[, 3] == 
                                       max(smry.temp3[, 3]), ]
  ...
  if (prune.ranefs) {
    ...
    if (length(ranef.to.remove) > 0) {
      ...
      eval(parse(text = paste("model<-update(model,.~.-", 
                              paste(rtr, collapse = "-"),"+",paste(rtr2, collapse = "+"), ",data=data)", sep = "")))
