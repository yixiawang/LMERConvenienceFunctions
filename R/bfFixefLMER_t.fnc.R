bfFixefLMER_t.fnc<-function(model,
	item=FALSE,
	method=c("t","llrt","AIC","BIC","relLik.AIC","relLik.BIC"),
	threshold=NULL,
	t.threshold=NULL,
	alphaitem=NULL,
	prune.ranefs=TRUE,
	...
	max(c(0,apply(as.data.frame(model@frame[,names(attributes(attributes(
	model@frame)$terms)$dataClasses[attributes(attributes(model@frame)$terms)$
	dataClasses=="factor"])]),MARGIN=2,FUN=function(x)length(levels(
	factor(x))))))>2){
	...
	if(is.null(threshold)){
		threshold<-c(2,0.05,5,5,4,4)[match(method[1],
			c("t","llrt","AIC","BIC","relLik.AIC","relLik.BIC"))]
	}

	# determine t.trheshold
	...
	if(is.null(log.file)){
		log.file<-file.path(tempdir(),paste("bfFixefLMER_t_log_",
			gsub(":","-",gsub("","_",date())),".txt",sep=""))
	}
  
	# set REML to FALSE if method != t and set.REML.FALSE = TRUE
		...
		eval(parse(text=paste("model.updated=update(model,.~.+(1|",
			...
			if(as.vector(anova(model,model.updated)[2,"Pr(>Chisq)"])<=alphaitem){
				cat("  log-likelihood ratio test p-value =",
					as.vector(anova(model,model.updated)[2,"Pr(>Chisq)"]),"\n")
	...
	if(as.vector(model@call[1])=="glmer()"){
		...
		data[,as.character(unlist(as.list(model@call))$formula[2])]=
			rnorm(nrow(data),0,1)
	...
	for(order in orders){
		...
		while(smry.temp2[smry.temp2[,3]==min(smry.temp2[,3]),3][1]<t.threshold){
			cat("  iteration", count, "\n")
			cat("    ",statistic,"for term",paste("\"",
				row.names(smry.temp2[smry.temp2[,3]==min(smry.temp2[,3]),
				])[1],"\"",sep=""),"=",smry.temp2[smry.temp2[,
				3]==min(smry.temp2[,3]),3][1],"<",t.threshold,"\n")
			# changed: check for occurence of the exact evaluated terms in 
			# each higher order interaction instead of checking occurence 
			# of the pattern -- Johannes Ransijn
			fac<-unlist(strsplit(gsub("\\)","",gsub("\\(","",
				as.character(row.names(smry.temp2[smry.temp2[,
				3]==min(smry.temp2[,3]),]))[1])),":"))
			hoi<-gsub("\\)","",gsub("\\(","",intr.order[intr.order$Order>order,
				"Coef"]))
			tt<-c()
			...
			if(length(grep("TRUE",tt))!=0){
				...
				keepers<-keepers[-grep(as.character(row.names(smry.temp2[
					smry.temp2[,3]==min(smry.temp2[,3]),]))[1],keepers)]
				...
				eval(parse(text=paste("m.temp=update(model,.~.-", 
          		...
          		if(method[1]=="llrt"){
            		...
            		if(as.vector(anova(model,m.temp)[2,"Pr(>Chisq)"])<=threshold){
            			...
            			keepers<-keepers[-which(keepers==as.character(row.names(
						smry.temp2[smry.temp2[,3]==min(smry.temp2[,3]),]))[1])]
            			smry.temp2<-smry.temp2[keepers,]
            			# a reduction term makes the script shorter (and easier 
						# to look through), compared to repeating the same 
						# commands -- Johannes Ransijn        
            			reduction<-FALSE
          			}else{
            			reduction<-TRUE
            			cat("    log-likelihood ratio test p.value =", 
        		...
        		if(reduction){
        			# remove term from model and get new summary
          			...
          			model<-m.temp
          			if (as.vector(model@call[1])=="glmer()"){
            			odv<-data[,as.character(unlist(as.list(model@call))$
							formula[2])]
            			data[,as.character(unlist(as.list(model@call))$
							formula[2])]<-rnorm(nrow(data),0,1)
            			temp.lmer<-update(model,.~.,family="gaussian",data=data)
            			coefs<-row.names(anova(temp.lmer))
            			data[,as.character(unlist(as.list(model@call))$
							formula[2])]<-odv
          			...
          			for(coef in coefs){
            			...
            			smry.temp3<-smry.temp3[smry.temp3[,3]==max(smry.temp3[,
							3]),]
          			...
          			keepers<-as.character(row.names(smry.temp2[
						smry.temp2$Order==order,]))
          			# smry.temp needs to be changed when the model 
					# is updated -- Johannes Ransijn
