          ...
          log.file = NULL) 
{
  ...
  for (order in orders) {
    ...
    if (smry.temp2[smry.temp2[, paste(p.value, ".p.val", 
                                      sep = "")] == max(smry.temp2[, paste(p.value, ".p.val", 
                                                                           sep = "")]), paste(p.value, ".p.val", sep = "")][1] < 
          alpha) {
      cat("  all terms of interaction level", order, "significant\n")
    }
    else {
      while (smry.temp2[smry.temp2[, paste(p.value, ".p.val",sep = "")] == max(smry.temp2[, paste(p.value,
                                                                                                  ".p.val", sep = "")]), paste(p.value, ".p.val", sep = "")][1] >= alpha) {
        cat("  iteration", count, "\n")
       
        if(
          length(smry.temp2[,paste(p.value, ".p.val", sep = "")] == max(smry.temp2[,
                                                                                   paste(p.value, ".p.val", sep = "")]))>1){
          evaluated.term <- as.character(row.names(smry.temp2[smry.temp2[, 
                                                                         3] == min(smry.temp2[, 3]), ]))
        }else{
        ...
        if (length(grep("TRUE", tt[, 1])) != 0) {
          ...
          if (method[1] %in% c("relLik.AIC", "relLik.BIC")) {
            ...
            m.temp.ic <- eval(parse(text = paste(ic, 
                                                 "(m.temp)", sep = "")))
            model.ic <- eval(parse(text = paste(ic, 
                                                "(model)", sep = "")))
  ...
  if (prune.ranefs) {
    ...
    for (vb in 1:length(split5)) {
      if (length(which(unlist(strsplit(split5[vb], "")) == 
                         "|")) > 0) {
